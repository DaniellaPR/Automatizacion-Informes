...
def save_docx(doc, file_name: str, folder_name: str = "Informe_General", puesto: str = "") -> Path:
    losFeos = '<>:"/\\|?*' # Se limpian los caracteres inválidos.
    safe_file = "".join(c for c in file_name if c not in losFeos).strip() or "informe.docx"
    safe_folder = "".join(c for c in folder_name if c not in losFeos).strip() or "Carpeta_Funcionario"
...
# la ifnormación general a colocar en la maoría de informes.
contexto_plantilla = {
...
while bandera == "s":
    ...
    # Se procede a mostrar el menú principal
    ...
    while bandera2 == "s":
        ...
        # ACLARACON: Esta opción se encuentra en desarrollo, en la presente versión, se genera el informe con todos los productos y actividades del TDR
            ...
            periodo_com= input("¿El periodo es completo? (s/n): ")
        ...
        # Se continúan con las selecciones del menú
        else:
            # Se da la opción de regresar a la sección anterior a la actual.
            if seleccion_menu == "4":
